...
const HEAD_BRANCH_REGEX = new RegExp('HEAD\\sbrancht:\\s(.+)', 'g');
...
const getMainBranch = async (flags) => {
  ...
  if (!regexResult) {
    console.error(`ERROR: Cannot find head branch! Use ${availableFlags.find((flag) => flag.name === flagNames.HEAD).keys[0]} flag to manually override head branch.`);
    return null;
  }
